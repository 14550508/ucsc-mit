...
class modDJMenuHelper {
    ...
    function aktywne(&$children) {
        ...
        if(is_object($menu->getActive())) foreach($children as $tab) {
        
            foreach ($tab as $obj) {
            
                if ($menu->getActive() && $obj->id == $menu->getActive()->id) return $obj->tree;
    ...
    function mosGetMenuLink($item, $level = 0, &$params, $havechild = null) {
		...
		// Add classes
        $classplus = '';        
        if (in_array($item->id, $this->tabakt)) {        
            $classplus = 'active ';
        }
		$classplus .= $item->anchor_css;
		$class = 'class="'.$classplus.'"';;
        if ($level == 0) $class = 'class="dj-up_a '.trim($classplus).'"';            
