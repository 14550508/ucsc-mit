...
<script type="text/javascript">	
	  ...
	  function enrolledQuizzData(key)
	  {	
        var user_id = Number('<?= $userData[0]->u_id; ?>');

	        $.ajax({
			    url:'<?= base_url() ?>quizzes/userEnrollQuiz',
			    type: "POST",
			    async: false, 
...
<script type="text/javascript">
    function enrolledQuizzView(Keyword)
	  {	
         
        var viewData = enrolledQuizzData(Keyword);
        ...
        var HtmlView ="";
	        ...
	        { 
	          ...
	          for(var i in Obj) 
	          {
	            HtmlView +='<div class="col-md-4"><section class="panel"><a href="<?= base_url() ?>quizzes/start/'+Obj[i].qz_id+'"  data-toggle="tooltip" data-placement="top" title="'+Obj[i].qz_name+'"><div class="pro-img-box">';
	         ...
	         $('#eqdLoad').html(HtmlView);
	        }

	        else{

	           $('#eqdLoad').html('No Results!');
...
<script type="text/javascript">
	$(document).ready(function() {
     	 ...
     	 $('#enrolledQuiz').keyup( function() 
     	 {
            var keyword =  $(this).val();
                 enrolledQuizzView(keyword);
     	 });
