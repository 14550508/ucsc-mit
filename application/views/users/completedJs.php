...
<script type="text/javascript">	
	  ...
	  function completedQuizzData(key)
	  {	
        var user_id = Number('<?= $userData[0]->u_id; ?>');

	        $.ajax({
			    url:'<?= base_url() ?>quizzes/userCompletedQuiz',
			    type: "POST",
			    async: false, 
...
<script type="text/javascript">
    function completedQuizzView(Keyword)
	  {	
        var user_id = Number('<?= $userData[0]->u_id; ?>');
        var viewData = completedQuizzData(Keyword);
        ...
        var HtmlView ="";
	        ...
	        { 
	          ...
	          for(var i in Obj) 
	          {
	            ...
	            HtmlView +='<div class="col-md-4"><section class="panel"><a href="<?= base_url() ?>quizzes/result/?quizID='+Obj[i].qz_id+'&attempts='+Obj[i].qz_attempt+'&user_id='+user_id+'"  data-toggle="tooltip" data-placement="top" title="'+Obj[i].qz_name+'"><div class="pro-img-box">';
	               
	               if(Obj[i].qz_into_image !=""){
	               		HtmlView +='<img src="<?= base_url() ?>uploads/profile_pic/'+Obj[i].qz_into_image+'" alt="" width="100%"></div>';  
	               }
	               else
	               {
	               ...
	               HtmlView +='<div class="text-center" style="height: 80px;"><h6>'+Obj[i].qz_name.substring(0,50)+'</h6><p><span class="label label-info">Points: '+Obj[i].points+'</span> <span class="label label-default">Attempt : '+Obj[i].qz_attempt+'</span></p>';               
	         ...
	         $('#cqdLoad').html(HtmlView);
	        }

	        else{

	           $('#cqdLoad').html('No Results!');
...
<script type="text/javascript">
	$(document).ready(function() {
     	 ...
     	 $('#completedQuiz').keyup( function() 
     	 {
            var keyword =  $(this).val();
                 completedQuizzView(keyword);
     	 });
