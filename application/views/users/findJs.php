...
<script type="text/javascript">	
	  ...
	  function findQuizzData(key, cat)
	  {	
        var user_id = Number('<?= $userData[0]->u_id; ?>');

	        $.ajax({
			    url:'<?= base_url() ?>quizzes/userFindQuiz',
			    type: "POST",
			    async: false, 
			    data:{key: key, user_id: user_id, cat: cat},
...
<script type="text/javascript">
    function findQuizzView(Keyword, cat)
	  {	
         
        var viewData = findQuizzData(Keyword, cat);
        ...
        var HtmlView ="";
	        ...
	        { 
	          ...
	          for(var i in Obj) 
	          {
	            HtmlView +='<div class="col-md-4"><section class="panel"><a href="<?= base_url() ?>quizzes/page/'+Obj[i].qz_id+'"  data-toggle="tooltip" data-placement="top" title="'+Obj[i].qz_name+'"><section class="panel"><div class="pro-img-box" >';
	         ...
	         $('#fqdLoad').html(HtmlView);
	        }

	        else{

	           $('#fqdLoad').html('No Results!');
...
<script type="text/javascript">
	$(document).ready(function() {
     	 ...
     	 $('#findQuiz').keyup( function() 
     	 {
            var keyword =  $(this).val();
            var cat =  $('#quizcat').val();
                 findQuizzView(keyword, cat);
     	 });


     	 $('#quizcat').on('change', function() {

        	 var cat = $(this).val(); 
        	 var keyword =  $('#findQuiz').val();  
        	 findQuizzView(keyword, cat);
